...
// Deliberately moderate: defensible against the $6.1M quarterly build and the
// derived days of inventory cover (DATA.md §3). Do not inflate.
export const IMPACT: Impact = {
  ...
  // Derived, never authored. RESOLVE reads this figure while PORTFOLIO and
  // RADAR read lib/derive/impact.ts directly, so a literal here is a number
  // that can disagree with itself across two screens. It used to say 52,
  // against a derived 51, because the bottleneck EXPOSED line is BOM-13 at
  // 41 weeks and the comment behind the 52 assumed 40.
  daysToHalt: daysToHalt(BOM.filter((b) => b.status === "EXPOSED")).daysToHalt,
