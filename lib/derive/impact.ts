...
// structure enough to justify a per-line weighting — see the BOM: the
...
// (grepped BOM/customer/actions for "coverage"/"onHand"/"inventory" — the
// only hits are prose, e.g. ActionImpact.tsx's own comment "52 — inventory
// cover at the moment of the event"). So days-to-halt is derived from the
// most lead-time-constrained EXPOSED line — the same "Longest Pole" concept
// LeadTimePressure.tsx already surfaces — combined with a stated buffer
// assumption. Two constants, both named so the formula reads as English:
//
...
//                                   itself isn't the one running out.
//
// FORMULA:
//   bottleneckWeeks = max(leadTimeWeeks) across exposed lines
//   overrunWeeks     = max(0, bottleneckWeeks − BASELINE_LEAD_TIME_WEEKS)
//   daysToHalt        = round(BUFFER_WEEKS × 7 − overrunWeeks × 0.6)
//
// Baseline: bottleneck is BOM-13 (SUB-BT-INF, substrate, MODELED, 40
// weeks — the same part LeadTimePressure.tsx's "Longest Pole" already
// names). overrun = 40 − 10 = 30 weeks. 70 − round(30 × 0.6) = 70 − 18 = 52
// days — an exact match to today's hardcoded figure, now computed rather
// than asserted.
...
// DURATION_HOP_DAYS of selected duration adds one more hop — a longer-
 ...
 *  — the same adjacency the graph screen's contamination BFS uses). */
 ...
 *  buildGraph) — strip the `G-` prefix to recover the BOM line id. */
