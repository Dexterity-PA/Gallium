...
| 0:38–0:55 | EXPOSURE | Click the ISO5852SDW row. **Hold.** | Drawer opens. Supply path. Then the ERP warning block. This is the beat the entire video exists for — hold it for a full six seconds, silent if needed. |
...
Write to the visuals, not over them. Roughly 150–170 words total at a calm pace. Leave the drawer beat and the graph sequence largely silent — let them land.
...
- [ ] `npm run build && npm start` — record the production build, not the dev server. Dev has HMR overhead and occasional flashes.
...
- [ ] All other applications closed — background CPU load causes animation jitter
...
Record the four screens as one continuous take if you can. Cutting between screens is acceptable but continuous is more convincing — it proves the app actually navigates.
...
That mapping is not public at part-number granularity. It is representative. The pattern it illustrates — US-branded parts with backend assembly in Taiwan, invisible to ERP country-of-origin fields — is real and common.
